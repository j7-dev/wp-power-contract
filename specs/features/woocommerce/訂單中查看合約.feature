...
Feature: 訂單中查看合約
  ...
  Rule: 前置（狀態）- WooCommerce 必須已啟用
    ...
    Example: WooCommerce 未安裝時訂單列表不顯示 Contract 欄位
      Given WooCommerce 未安裝
      When 管理員進入訂單列表頁面
      Then 訂單列表不包含 "Contract" 欄位

  Rule: 後置（回應）- 訂單列表應顯示關聯合約 ID 連結

    Example: 訂單有多筆關聯合約時顯示所有合約連結
  ...
  Rule: 後置（回應）- 訂單無關聯合約時 Contract 欄位應為空

    Example: 訂單沒有合約時欄位為空
      Given 系統中有 WooCommerce 訂單 #502，無關聯合約
      Then 訂單 #502 的 "Contract" 欄位為空

  Rule: 後置（回應）- 應同時支援傳統訂單列表與 HPOS 訂單列表

    Example: HPOS 啟用時訂單列表仍顯示合約欄位
      Given WooCommerce 啟用 HPOS（High-Performance Order Storage）
      And 管理員 "Admin" 在 WooCommerce 訂單列表頁面
      Then 訂單列表包含 "Contract" 欄位
